...
// --- Logic Check In / Resume ---
const handleCheckIn = async (lead) => {
    selectedLeads.value = lead;

    // Jika di state sudah ada ID-nya (karena baru klik atau habis refresh lalu klik lagi)
    const res = await dataLeadsVisit.checkInVisit(lead.id);
    ...
    if (res.success) {
        ...
        const modal = bootstrap.Modal.getOrCreateInstance(document.getElementById("modal-input-visit"));
        modal.show();
    } else {
...
//   // 1. Gunakan FormData karena ada upload file (foto)
//   const formData = new FormData();
  
//   // Karena route Laravel Anda adalah PUT, kita gunakan spoofing method
//   formData.append('_method', 'PUT'); 

//   // 2. Masukkan semua data yang dibutuhkan backend
//   formData.append('latitude', latitude.value);
//   formData.append('longitude', longitude.value);
//   formData.append('gps_snapshot', address.value);
//   formData.append('notes', form.notes);
//   formData.append('customer_response', form.status);
//   formData.append('visit_result', form.notes); // Sesuaikan jika ada input lain

//   if (photoBlob.value) {
//     formData.append('photo', blobToFile(photoBlob.value, `visit-${Date.now()}.jpg`));
//   }

//   // 3. Panggil action di Pinia Store
//   const res = await dataLeadsVisit.storeVisitReport(formData);

//   if (res.success) {
//     // Tampilkan notifikasi sukses
//     await Swal.fire({
//       icon: "success",
//       title: "Visit Berhasil Diselesaikan",
//       text: "Laporan tersimpan dan status kunjungan telah di-reset.",
//       timer: 1500,
...
//     // 5. Reset tampilan (kamera & lokasi) dan refresh data tabel
//     resetVisitState();
//     await dataLeadsVisit.fetchLeadsVisitStore();
//   } else {
//     // Tangani jika ada error validasi dari Laravel
//     if (res.errors) {
...
const submitVisit = async () => {
  ...
  try {
    await dataLeadsVisit.storeVisitLead({
      lead_id: selectedLeads.value.id,
      ...
      photo: blobToFile(photoBlob.value, `visit-${Date.now()}.jpg`)
    });
    ...
    await Swal.fire({
      icon: "success",
      title: "Success",
      text: "Visit activity saved successfully",
      timer: 1200,
      showConfirmButton: false,
    });
    ...
    // ✅ Redirect ke halaman sales-visit
   router.push("/sales-visit");
    ...
    if (err.response?.status === 422) {
      ...
      Swal.fire({
        icon: "error",
        title: "Failed",
        text: err.response?.data?.message || "Failed to save visit",
      });
    }
  }
};
...
onMounted(async () => {
  ...
  // --- AMBIL DATA DARI STORAGE AGAR TOMBOL TIDAK DISABLED ---
  const savedLeadId = localStorage.getItem("active_visit_lead_id");
  const savedVisitId = localStorage.getItem("active_visit_id");
  
  if (savedLeadId && savedVisitId) {
      dataLeadsVisit.activeLeadId = parseInt(savedLeadId); // Pastikan tipenya Number jika ID adalah angka
      dataLeadsVisit.activeVisitId = parseInt(savedVisitId);
  }
...
// Logika Tombol Visit Now
// const handleVisitNow = async (lead) => {
//     const res = await dataLeadsVisit.checkInVisit(lead.id);
//     if (res.success) {
//         // Simpan manual jika di store belum ada logic ini
//         localStorage.setItem("active_visit_lead_id", lead.id);
//         localStorage.setItem("active_visit_id", dataLeadsVisit.activeVisitId);
        
//         Swal.fire("Berhasil", "Waktu kedatangan dicatat. Silahkan lanjut ke Check-in.", "success");
//     } else {
//         Swal.fire("Gagal", res.message, "error");
//     }
// };
...
const handleVisitNow = async (lead) => {
    const res = await dataLeadsVisit.checkInVisit(lead.id);
    if (res.success) {
        // PENTING: Isi state ini agar UI langsung berubah tanpa refresh
        dataLeadsVisit.activeLeadId = lead.id;
        dataLeadsVisit.activeVisitId = dataLeadsVisit.activeVisitId; // ID dari store

        // Simpan ke storage untuk backup refresh
        localStorage.setItem("active_visit_lead_id", lead.id);
        localStorage.setItem("active_visit_id", dataLeadsVisit.activeVisitId);

        Swal.fire("Berhasil", "Silahkan lanjut ke Check-in.", "success");
    }
};

// Logika Tombol Check-in (Buka Modal)
                   ...
                   <tr>
                      <td colspan="8" class="text-center">
                        <div class="d-flex flex-column align-items-center justify-content-center">
                          ...
                          Data Leads Ready To Visit Not Found
                          </p>
                        </div>
                      </td>
                    </tr>
                ...
                <tbody v-else>
                  ...
                  <td>
                   ...
                   <td>
                      ...
                      >
                        <i class="fa-solid fa-street-view"></i> Visit Now (Ongoing)
                      </button>
  ...
  {{ errors.customer_response[0] }}
          ...
          <div class="modal-footer">
            ...
            <button
              ...
              :disabled="isSubmitDisabled || dataLeadsVisit.savingLeadVisit"
              @click="submitVisit"
            >
              <i class="fa-solid fa-cloud-arrow-up me-2"></i>
              {{ dataLeadsVisit.savingLeadVisit ? 'Saving...' : 'Check Out & Submit Visit Leads' }}
            </button>
          </div>
        </div>
      </div>
