...
class SubmenuValidationIndex extends FormRequest
{
    protected array $allowedSortFields = ['title','created_at'];
    ...
    if (count($unknown) > 0) {
        throw new HttpResponseException(response()->json([
            ...
            'errors' => collect($unknown)->mapWithKeys(
                fn ($f) => [$f => ['Field ini tidak valid']]
            )
        ], 422));
    }

    if ($this->has('only_deleted')) {
        $this->merge([
            'only_deleted' => filter_var(
                $this->input('only_deleted'),
