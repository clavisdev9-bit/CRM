    ...
    private function buildFollowUpDetail($id)
    {
        return DB::table('follow_ups as fu')
            ->select([
                ...
                // timestamp LENGKAP (tanggal+jam) dari follow_up_at, dipakai frontend
                // buat card "Follow Up Selanjutnya" (format via store.formatDate()) --
                // beda dari 'scheduled_time' di atas yang cuma jam doang (HH:MI),
                // makanya sebelumnya kelihatan aneh (jam tanpa tanggal).
                'fu.follow_up_at as next_follow_up_at',
                'fu.follow_up_type',
                'fu.subject',
                'fu.notes',
                'fu.status',
